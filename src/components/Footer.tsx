          ...
          <div className="flex flex-wrap justify-center gap-6">
            {[Instagram, Twitter, Linkedin, Facebook].map((Icon, i) => (
              <a 
                key={i}
                href="#" 
                className="w-12 h-12 rounded-full bg-white/10 border border-white/20 flex items-center justify-center hover:bg-brand-accent hover:border-brand-accent transition-all group"
              >
          ...
          <nav className="pt-10">
            <ul className="flex flex-wrap justify-center gap-x-12 gap-y-4">
              {[
                ...
                <li key={link.name}>
                  <a href={`#${link.id}`} className="text-sm font-medium uppercase tracking-widest text-brand-gray hover:text-white transition-colors relative group">
                    {link.name}
